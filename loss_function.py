...
P_d = 1 # downlink power: 200 mW
p_u = 0.1 # uplink power: 100 mW
p_p = 0.5 # pilot power: 100mW
noise_figure = 9 #dB
...
def dl_rate_calculator_w_pilot_index(pilot_index_torch, beta_torch, tau_p):
    ...
    for k in range(num_ue):
        ...
        numerator = torch.sqrt(torch.tensor(rho_p * tau_p, dtype=torch.float64)) * beta_torch[:, k]
        denominator = tau_p * rho_p * sum_val + 1
        c_mk[:, k] = numerator / denominator
    #gamma
    gamma = torch.sqrt(torch.tensor(rho_p * tau_p, dtype=torch.float64)) * beta_torch * c_mk
    #eta
    etaa = 1 / torch.sum(gamma, dim=1)
    eta = etaa[:, None].expand(-1, num_ue)
    #sinr
    ...
    flag = (pilot_index_torch[:, None] == pilot_index_torch)
    flag = flag.float().to(dtype=torch.int64)
    for k in range(num_ue):
...
def dl_rate_calculator_w_pilot_probs(pilot_probs, beta, tau_p, epoch, batch_no,sample):
    ...
    numerator_c = torch.sqrt(torch.tensor(rho_p * tau_p, dtype=torch.float64)) * beta  # Shape: (M, K)
    denominator_c = tau_p * rho_p * inner_sum + 1
    c_mk = numerator_c / denominator_c  # Shape: (M, K)
    gamma = torch.sqrt(torch.tensor(tau_p * rho_p, dtype=torch.float64)) * beta * c_mk
    # etaa = 1 / torch.sum(gamma, dim=1)
    ...
    flag = flag.float().to(dtype=torch.float64)
    for k in range(num_ue):
        #
        # mask = torch.arange(num_ue, device=beta.device) != k
        # sum_squared = torch.sum(
...
def loss_function_PA(args, beta, output, epoch, batch_no):
    ...
    torch_one_hot = F.one_hot(pilot_index, num_classes = args.pilot_length)
    ...
    # print(f'------EPOCH: {epoch} ------ SAMPLE: {batch_no}------ \n{pilot_probs[0]}')
    # print(f'pilot_probs_process: \n{pilot_probs_process[0]}')
    for i in range(args.batch_size):
        ...
        sum_rate_each_batch[i] = torch.sum(
            dl_rate_calculator_w_pilot_probs(pilot_probs_process[i], beta[i], args.pilot_length, epoch, batch_no, i))
